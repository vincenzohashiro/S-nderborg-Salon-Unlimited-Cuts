...
const Booking = () => {
  const { general, seo, pages } = useSiteConfig();
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="pt-40 pb-16 bg-gradient-dark text-background text-center">
        <div className="container">
          <span className="text-xs uppercase tracking-[0.3em] text-gold">{pages.booking.badge}</span>
          <h1 className="font-serif text-5xl md:text-7xl mt-4 text-balance">
            {pages.booking.heading1} <span className="italic text-gold">{pages.booking.heading2}</span>
          </h1>
          <p className="mt-6 max-w-xl mx-auto text-background/80 font-light">
            {pages.booking.subtext}
          </p>
        </div>
      </section>
        ...
        <div className="container max-w-5xl">
          ...
          <div className="bg-secondary border border-border rounded-sm overflow-hidden shadow-elegant">
            <div className="p-6 border-b border-border flex items-center justify-between flex-wrap gap-4">
              <div>
                <h2 className="font-serif text-2xl">{pages.booking.iframeHeading}</h2>
                <p className="text-sm text-muted-foreground">{pages.booking.iframeSubtext}</p>
              </div>
              <Button variant="gold" asChild>
                <a href={general.planwayUrl} target="_blank" rel="noopener noreferrer">
                  {pages.booking.iframeCta} <ExternalLink className="ml-2 w-4 h-4" />
                </a>
              </Button>
            </div>
            <iframe
